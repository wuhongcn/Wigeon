...
if(0) then    
  ...
  ! write(*,*) hostname,myid;
if (nLx>1.and.nLy>1.and.nLz>1) call fghx_gpuweno(uc,rhs,ark,io,gamma,nLx,nLy,nLz,mnp,myid) 
     if(myid.eq.0)call CPU_screentime(ts,cpu_weno_nocom,'time in WENO no com')
...
!$omp end parallel do
te=mpi_wtime()
if(myid.eq.0) write (*,*) "fx time:", te-ts
...
enddo
te=mpi_wtime()
if(myid.eq.0) write (*,*) "gy time:", te-ts
!$omp end parallel do 
...
enddo
te=mpi_wtime()
if(myid.eq.0) write (*,*) "hz time:", te-ts
